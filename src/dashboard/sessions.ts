...
export function createDashboardRuntime(options: CreateDashboardRuntimeOptions) {
  ...
  const runtime = {
    ...
    probeVisionCapability: (input: DashboardRequestInput = {}, request: { signal?: AbortSignal } = {}) => runtimeProbeVisionCapability(ctx, input, request),
