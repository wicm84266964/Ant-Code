...
export function createCredentialStore({ filePath }) {
  ...
  return Object.freeze({
     ...
     */
    async set(reference, secret, options = {}) {
      validateReference(reference);
...
/** @param {string} target */
async function ensureCredentialDirectory(target) {
...
function assertExpectedRevision(expected, actual) {
...
function credentialMap(document) {
  return isPlainObject(document.credentials) ? document.credentials : {};
}

/** @param {Record<string, any>} document */
function copyCredentialMap(document) {
 ...
 */
function mutationDescriptor(written, reference, configured) {
  return {
    path: written.path,
    exists: true,
