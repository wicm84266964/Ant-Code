...
import { createFileRepository, ConfigRevisionConflictError } from "./file-repository.js";
import { migrateV1Documents, stripLegacyModelFields } from "./migrate-v1.js";
import {
  credentialsPath,
...
import { resolveSettingsLayers } from "./resolver.js";
import { isV2SettingsDocument, validateSettingsDocument } from "./schema.js";
 ...
 */
export async function ensureConfigV2(options = {}) {
  ...
  const settingsWrites = {
    global: globalExists
      ...
      : await repositories.globalSettings.replace(globalDocument, {
          expectedRevision: snapshots.globalSettings.revision
        }),
    project: projectExists
      ? { revision: snapshots.projectSettings.revision }
      : await repositories.projectSettings.replace(projectDocument, {
  ...
  for (const scope of ["global", "project"]) {
    ...
    await markerRepository.replace({
      ...
      credentialRefs: ownedCredentialReferences(scope === "global" ? globalDocument : projectDocument)
 ...
 */
export async function rollbackConfigV2(input) {
  ...
  for (const entry of backups) {
    ...
    const backup = path.resolve(String(entry?.backup ?? ""));
    ...
    await createFileRepository({ filePath: source }).replace(data);
  }

  const credentialPath = String(input?.paths?.credentials ?? "").trim();
  ...
  if (credentialRefs.length > 0) {
    ...
    for (const reference of credentialRefs) await store.clear(reference);
  }
  ...
  return {
    ok: true,
    restored: backups.map((entry) => entry.source),
...
function migrationPaths(cwd, env) {
  return {
...
function assertDistinctScopePaths(paths) {
  if (samePath(paths.globalSettings, paths.projectSettings)) {
    const error = new Error("Global and project Config V2 settings paths must be distinct");
...
async function readMigrationSnapshots(repositories) {
  const entries = await Promise.all(Object.entries(repositories).map(async ([name, repository]) => (
    [name, await repository.read()]
  )));
  return Object.fromEntries(entries);
}

/** @param {Record<string, any>} snapshot @param {string} scope */
function assertSettingsSnapshot(snapshot, scope) {
  if (snapshot.exists && !isV2SettingsDocument(snapshot.data)) {
    const error = new Error(`${scope} settings.json is not a valid Config V2 document`);
...
async function planCredentialWrites(store, credentials) {
  const plan = [];
  for (const [reference, secret] of Object.entries(credentials)) {
    ...
    if (current !== secret) {
      const error = new Error(`Credential reference ${reference} already contains a different value`);
...
async function createMigrationBackups(snapshots, changes) {
  const backups = [];
  for (const scope of ["global", "project"]) {
...
function ownedCredentialReferences(document) {
...
function snapshotRevisions(snapshots, credentialRevision) {
  return {
...
function migrationIdentifier(...revisions) {
...
function migrationResult(value) {
