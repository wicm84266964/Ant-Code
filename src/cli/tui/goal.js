 ...
 * @param {string[]} args
 */
export function parseTuiGoalCommand(args) {
 ...
 * @param {string} status
 */
export function tuiGoalStatusLabel(status) {
  return {
    active: "进行中",
    running: "进行中",
    paused: "已暂停",
 ...
 */
export function formatTuiGoalFooter(session) {
 ...
 * @param {Record<string, any>} session
 */
export function formatTuiGoalStatus(session) {
  const goal = session?.goal;
  if (!goal?.enabled) {
    return [
      "Goal 未开启。",
 ...
 * @param {Record<string, any>} session
 */
export function formatTuiGoalRecapEntry(session) {
  const goal = session?.goal;
  if (!shouldShowGoalRecap(goal)) {
    return null;
  }
...
export async function finishTuiGoalTurn(input) {
  ...
  let plan = { continue: false, recap: false };
...
export function shouldSkipTuiGoalQuestion(session) {
...
/** @param {Record<string, any>} session @param {NodeJS.ProcessEnv} [env] */
async function persistTuiGoal(session, env) {
  if (!session?.id) {
    return;
  }
  try {
