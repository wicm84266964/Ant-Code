...
export async function buildInitialContext(options) {
  ...
  return {
    system: [
      ...
      "- For long-running terminal commands that should continue while the user keeps chatting, use background_shell instead of powershell/bash or hand-written Start-Process/nohup. It registers PID, logs, status, and Dashboard cancellation; report the task id and log paths, then end the turn promptly. If a discover/download/render task is missing or stopped and needs to be restarted, restart it with background_shell again; never fall back to foreground powershell/bash for that job.",
      ...
      "- /compact is a context-window operation that prefers a separate model summarization request through the configured gateway; if the gateway is unavailable or summarization fails, it falls back to local deterministic compaction.",
      ...
      "- MCP is optional. Use mcp_list to inspect configured servers/tools; mcp_call works only when a local or approved MCP server is configured, and missing MCP servers do not disable built-in local tools.",
      ...
      "- /agents and agent_run provide controlled local subagents. They use the same configured model gateway, a profile-specific tool allowlist, and the parent session permission engine.",
      ...
      "- readonly subagents can fall back to deterministic local scanning when no gateway is configured; verifier and code-worker profiles require the configured gateway for model-driven work.",
      "",
      ...
      "- Keep final answers concise and actionable for project users who may not be professional testers.",
      "",
