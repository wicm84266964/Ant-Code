...
export async function buildInitialContext(options: { cwd: string; config: import("../config/load-config.ts").LabAgentConfig; env?: NodeJS.ProcessEnv; clientSurface?: string }) {
  ...
  return {
    system: [
      ...
      "- waitForGroup and wakeParent apply to the entire group. Set the same values on every task in a shared group; do not disable wakeup on the first task and enable it only on the last. Never end your turn expecting automatic continuation from a group with wakeParent=false or waitForGroup=none.",
