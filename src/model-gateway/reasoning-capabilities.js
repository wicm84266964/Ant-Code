import { normalizeReasoningEfforts } from "./models.js";

const PROBE_EFFORT_IDS = Object.freeze(["none", "off", "low", "medium", "high", "xhigh", "max", "ultra"]);
const LEGACY_GPT56_ULTRA_PRESET_IDS = Object.freeze(["low", "medium", "high", "xhigh", "max", "ultra"]);
...
const KNOWN_PRESETS = Object.freeze([
  Object.freeze({
    id: "xai.grok-4.5-4.6",
 ...
 */
export function inferCatalogReasoning(rawModel, context = {}) {
 ...
 */
export function isLegacyGpt56UltraPreset(modelId, protocol, efforts) {
...
/** @param {unknown} value */
export function normalizeCapabilityEfforts(value) {
...
function capabilityResult(input) {
  ...
  return {
    reasoningEfforts: efforts.map(({ id, label, description }) => ({ id, label, description })),
...
/** @param {unknown} value */
function defaultEffortEntryId(value) {
  let entries = value;
  if (typeof entries === "string") {
    try {
  ...
  if (!Array.isArray(entries)) return "";
  const entry = entries.find((candidate) => isPlainObject(candidate) && candidate.default === true);
