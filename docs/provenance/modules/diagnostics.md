...
design_notes:
  - Checks Node version, CLI bin wiring, config sources, gateway configuration, network mode, allowed hosts, metadata retention/encryption, MCP configuration, provenance files, source provenance release attestation, release audit material, local installation material, release candidate package material, RC acceptance material, and lab user quickstart material.
