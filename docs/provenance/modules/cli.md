...
references:
  ...
  - open_source: vscode-ripgrep wrapper, MIT licensed, public npm package
  ...
  - open_source: TypeScript compiler and language service, Apache-2.0 licensed, public npm package
  - open_source: unpdf PDF text extraction library, MIT licensed, public npm package
  - open_source: napi-rs canvas PDF page rendering backend, MIT licensed, public npm package
...
design_notes:
  ...
  - Interactive chat is a local terminal loop with session state, todo/plan display, context budget status, delivery status footers with validation suggestions, review/next/report commands, local resume, context clearing/compaction, and per-tool approval prompts.
  ...
  - The TUI keeps resumed visible transcript memory bounded while allowing `--resume` to request archived transcript restoration for model context through the shared session runtime.
  - Stage D-H TUI work adds independent slash command discovery, workspace file mentions, local shell composer mode, queued prompts during active turns, permission-mode cycling, richer approval modal text, command output runtime-log routing, and local disabled-state surfaces for deferred advanced workflows.
