...
design_notes:
  ...
  - Implements local ripgrep-backed `rg_search`, `rg_files`, `rg_files_with_matches`, and `rg_count` handlers for regex/glob/context search while preserving the legacy grep handler as fallback.
  - Implements local TypeScript/JavaScript semantic handlers for symbols, diagnostics, definitions, and references using the bundled TypeScript language service.
