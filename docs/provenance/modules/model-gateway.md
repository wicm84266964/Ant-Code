...
design_notes:
  ...
  - Repairs restored local history before OpenAI Chat Completions conversion so assistant tool-call records are forwarded only when their matching local tool results are present.
  ...
  - Local model aliases are represented as independent gateway configuration data; the default lab adapter options are the three locally provided Ant Code aliases, and no provider account/model-list endpoint is called.
