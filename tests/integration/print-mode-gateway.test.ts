...
import { runPrintTurn } from "../../src/core/session.ts";
 ...
 * @param {string} host
 */
function listen(server: import("node:http").Server, host: string) {
 ...
 * @param {import("node:http").Server} server
 */
function serverUrl(server: import("node:http").Server) {
 ...
 */
function openAITextResponse(body: Record<string, unknown>, content: string) {
  return {
    id: "chatcmpl-text",
    model: body.model,
    choices: [
 ...
 */
function mockGatewayEnv(url: string, model: string, extra: Record<string, string> = {}) {
  return {
