...
import { createPrintEventCollector } from "../../src/cli/print-output.ts";
import { isPrintableAntEvent } from "../../src/core/events.ts";
import { reduceAntEvents } from "../../src/core/event-reducer.ts";
import { createSession, runPrintTurn, runSessionTurn } from "../../src/core/session.ts";
 ...
 */
function createOpenAIStreamingGateway(requests: Array<Record<string, unknown>>) {
 ...
 * @param {string} host
 */
function listen(server: http.Server, host: string) {
 ...
 * @param {string} url
 */
function openAIStreamEnv(url: string) {
  return {
