 ...
 * @param {Array<Record<string, any>>} requests
 */
function createDeepSeekReasoningToolGateway(requests: Array<Record<string, unknown>>) {
  return http.createServer(async (request, response) => {
 ...
 */
function createRepeatedToolGateway(requests: Array<Record<string, unknown>>, fixture: { toolRounds: number; finalText: string; toolName?: string; input?: Record<string, unknown>; usage?: Record<string, unknown> }) {
  return http.createServer(async (request, response) => {
    if (request.method !== "POST" || request.url !== "/v1/chat") {
      response.writeHead(404, { "content-type": "application/json" });
      response.end(JSON.stringify({ error: { code: "NOT_FOUND" } }));
 ...
 */
function createMultiToolThenFinalGateway(requests: Array<Record<string, unknown>>, fixture: { finalText: string }) {
  return http.createServer(async (request, response) => {
    if (request.method !== "POST" || request.url !== "/v1/chat") {
      response.writeHead(404, { "content-type": "application/json" });
      response.end(JSON.stringify({ error: { code: "NOT_FOUND" } }));
 ...
 */
function createSequenceToolGateway(requests: Array<Record<string, unknown>>, fixture: { toolCalls: Array<Record<string, unknown>>; finalText: string }) {
  return http.createServer(async (request, response) => {
    if (request.method !== "POST" || request.url !== "/v1/chat") {
      response.writeHead(404, { "content-type": "application/json" });
      response.end(JSON.stringify({ error: { code: "NOT_FOUND" } }));
 ...
 */
function readRequestJson(request: http.IncomingMessage) {
 ...
 */
function mockGatewayEnv(url: string, options: { protocol?: string } = {}) {
