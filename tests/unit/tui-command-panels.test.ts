...
import { compactSessionContext, createContextWindow, summarizeContextWindow } from "../../src/core/context-window.ts";
import {
  ...
  createUndoRedoPanel,
  createUsagePanel
} from "../../src/cli/tui/command-panels.ts";
...
test("/compact panel reflects real context compaction result", () => {
  ...
  assert.match(contextText, /当前模型上下文：[\d.]+k?\/200k tokens（估算）/);
  assert.match(contextText, /模型消息链：[\d.]+k? tokens（估算）/);
