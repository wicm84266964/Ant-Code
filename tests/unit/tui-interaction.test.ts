...
import { persistTuiPermissionCycle } from "../../src/cli/tui/permission-cycle.ts";
import { enableGoalState } from "../../src/core/goal.ts";
...
import {
  ...
  topPopover
} from "../../src/cli/tui/interaction.ts";
...
test("TUI permission cycle refuses to switch while Goal is enabled", async () => {
  ...
  try {
    const { runSessionTurn } = await import("../../src/core/session.ts");
    ...
    const { persistSessionSnapshot } = await import("../../src/core/session.ts");
    ...
    await assert.rejects(
      ...
      (error: { code?: string }) => error?.code === "GOAL_PERMISSION_LOCKED"
    );
