...
import { parseSlashCommand } from "../../src/commands/parser.ts";
import { runSlashCommand } from "../../src/commands/runtime.ts";
import { clearHookAudit } from "../../src/hooks/audit-store.ts";
import { runHooks } from "../../src/hooks/runner.ts";
import { registerBackgroundAgentTask } from "../../src/agents/background-registry.ts";
...
import { trustWorkspace } from "../../src/permissions/workspace-trust.ts";
import { createWorkflowState } from "../../src/tools/workflow-tools.ts";

const execFileAsync = promisify(execFile);
const CONFIG_FREE_REPO_CWD = path.join(process.cwd(), "tests");
...
test("status slash command reports local session settings", async () => {
  ...
  assert.equal(status.networkMode, "lab-only");
});
...
test("keybindings expose TUI discovery states", async () => {
  ...
  assert.match(keybindings, /拖选后复制：拖选可见聊天文字复制到系统剪贴板/);
  assert.match(keybindings, /←\/→：输入为空时切换右侧栏分类/);
  assert.match(keybindings, /Ctrl\+L：清屏：只隐藏当前可见记录，不删除会话文件/);
...
test("model command explains local gateway-owned model selection", async () => {
  ...
  assert.doesNotMatch(output, /mimo-v2\.5 thinking context=400k/);
      ...
      servers: [
        {
          ...
          args: [path.resolve("tests/fixtures/mcp-echo-server.ts")],
          toolRisks: { echo: "read" }
        }
      ]
    }
