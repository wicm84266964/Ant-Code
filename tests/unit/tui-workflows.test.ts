...
import {
  ...
  takeQueuedPrompt
} from "../../src/cli/tui/workflows.ts";

test("TUI forwards the active turn signal to shell and slash commands", async () => {
  const appDir = path.resolve("src/cli/tui");
  const files = await fs.readdir(appDir);
  const sources = await Promise.all(
    files
      .filter((name) => name === "app.ts" || name.startsWith("app-"))
      .map((name) => fs.readFile(path.join(appDir, name), "utf8"))
  );
