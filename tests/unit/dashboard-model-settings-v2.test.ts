...
import {
  ...
  saveV2DefaultModel,
  saveV2ProviderModel
} from "../../src/dashboard/model-settings-v2.ts";
...
test("Dashboard V2 model mutations honor the transaction lock across processes", async () => {
  ...
  const lockTarget = path.join(fixture.home, ".ant-code", "model-settings-v2.transaction");
  const configStoreUrl = new URL("../../src/dashboard/config-store.ts", import.meta.url).href;
...
async function runCredentialCrashChild(operation, options, stage) {
  const modelSettingsUrl = new URL("../../src/dashboard/model-settings-v2.ts", import.meta.url).href;
...
function crossScopeDeletionSettings(activeProvider: "active" | "legacy") {
  return {
    settingsVersion: 2,
    namespaces: {
      "model-providers": {
...
function waitForChildOutput(child: import("node:child_process").ChildProcessWithoutNullStreams, expected: string) {
