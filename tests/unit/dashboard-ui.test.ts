...
import { visibleTranscriptRole } from "../../src/dashboard/public/transcript.ts";
...
async function loadAppExports(exportNames) {
  const source = await publicTsSource();
...
test("dashboard distinguishes configuration failures from service and network failures", async () => {
  const module = await loadAppExports(["bootstrapFailurePresentation"]);
...
test("dashboard prioritizes transient gateway retry over background status", async () => {
  ...
  const module = await loadAppExports(["primaryLiveActivity", "liveStatusTitle"]);
...
test("dashboard app keeps directory paths when linkifying file references", async () => {
  ...
  const module = await loadAppExports(["replaceFileReferences"]);
...
test("dashboard app keeps event streams and drafts scoped to the active turn", async () => {
  ...
  assert.match(source, /function beginEventTurn\(/);
...
test("dashboard app exposes session actions and reconnects active sessions", async () => {
  ...
  assert.match(source, /function scheduleSessionsRefresh\(delayMs/);
  ...
  assert.match(source, /await openSession\(sessionId, \{ restore: !listed \}\)/);
  assert.match(source, /export async function openSession\(id: string, options: \{ restore\?: boolean \} = \{\}\)/);
  assert.match(source, /options\.restore !== true/);
  assert.match(source, /if \(activity\.status === "failed" \|\| activity\.status === "blocked"\) \{/);
  ...
  assert.match(source, /const hasBackground = restoreBackgroundSnapshot\(loadedSession\.backgroundSnapshot\)/);
  assert.match(source, /loadedSession\.active && loadedSession\.running/);
  assert.match(source, /loadedSession\.active && hasBackground/);
  assert.match(source, /rememberEventCursor\(loadedSession\.eventCursor\)/);
...
test("dashboard app guards duplicate turn submission and scopes permission by session", async () => {
  ...
  assert.match(source, /applyGoalSnapshot\(result\.goal \?\? state\.goal, \{ permissionMode: result\.permission\?\.mode \?\? state\.permissionMode \}\)/);
  assert.match(source, /state\.running = loadedSession\.active === true && loadedSession\.running === true/);
...
test("dashboard keeps max and ultra opt-in and renders one configured disabled effort alias", async () => {
  ...
  const { reasoningEffortCatalog } = await loadAppExports(["reasoningEffortCatalog"]);

  assert.match(app, /normalizeReasoningEfforts\(modelInfo\?\.reasoningEfforts\)/);
  assert.match(app, /efforts\.map\(\(effort\) =>/);
  assert.deepEqual(
...
test("dashboard resolves legacy model ids as atomic provider and model selections", async () => {
  const app = await publicTsSource();
  const { resolveAtomicModelSelection } = await loadAppExports(["resolveAtomicModelSelection"]);
  const grok = {
    id: "grok",
    modelAlias: "grok-4.6",
    current: true,
...
test("dashboard renders context compaction as a transcript divider", async () => {
  const app = await publicTsSource();
  const events = await fs.readFile(path.resolve("src/dashboard/events.ts"), "utf8");
  const css = await fs.readFile(path.resolve("src/dashboard/public/styles.css"), "utf8");
...
test("dashboard keeps background subagent status visible after the main turn ends", async () => {
  ...
  const sessions = await fs.readFile(path.resolve("src/dashboard/sessions.ts"), "utf8");
  const runtimeFiles = await fs.readdir(path.resolve("src/dashboard/runtime"));
  const runtimeSource = (await Promise.all(runtimeFiles.filter((name) => name.endsWith(".ts")).map((name) => fs.readFile(path.resolve("src/dashboard/runtime", name), "utf8")))).join("\n");
  assert.match(sessions + runtimeSource, /startsWith\("background_terminal_"\)/);
  ...
  assert.match(app, /function restoreBackgroundSnapshot\(/);
...
test("dashboard renders lightweight office previews in the side panel", async () => {
  ...
  assert.match(app, /function renderOfficePreview\(/);
  assert.match(app, /function renderTablePreview\(/);
  assert.match(app, /function showTableLightbox\(/);
  assert.match(app, /function renderExpandedTableHtml\(/);
...
test("dashboard responsive, composer, focus, and scroll helpers enforce UI behavior", async () => {
  const module = await loadAppExports([
    "dashboardFetch",
    "normalizedResponsiveView",
    "contextActionRequestOptions",
    "composerHeightFor",
    "previewWidthBounds",
    "clampedPreviewWidth",
...
test("dashboard qualifies model deletion state by provider and classifies only revision conflicts", async () => {
  const module = await loadAppExports(["state", "settingsModelHtml", "providerModelKey", "isConfigRevisionConflict"]);
