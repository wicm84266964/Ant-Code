...
- 分层验证记忆：`/verify suggest` 会把本地检查分成 `minimal`、`related`、
  `full` 三层，`/verify`、`/next`、`/report` 会显示当前会话里的 pending、
  passed、failed、stale 验证状态。
...
- 本地 ripgrep 搜索工具：支持正则、glob、上下文、文件列表、命中文件和计数
- 本地 TypeScript/JavaScript 语义工具：支持符号、诊断、定义跳转和引用查找
- 带审批边界的本地 Shell 执行
- 可配置模型网关和健康检查
...
（`@vscode/ripgrep`）和 TypeScript language service。用户不需要再单独运行搜索
或 TypeScript 的安装脚本。
